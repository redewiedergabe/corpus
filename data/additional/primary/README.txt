...
Folder "consensus" contains the same text samples and consensus annotation as the core corpus (data/main). In contrast to the core corpus, the text samples are tokenized with OpenNLP so that they can be aligned with the annotations in the planet folders. (NOTE: The xmi files of the main corpus contain OpenNLP Tokens as well (Annotation "Token"), so they can also be used for alignment.)
